...
For a path like `/_v/validate/:code`, if you receive the request `/_v/validate/200`, `ctx.vtex.params` will return `{ code: '200' }`
...
When building a HTTP service, we should follow HTTP rules regarding data types, cache, authorization, and validate code. Our example app sets a `ctx.validate` value that will be used as a HTTP validate code return value, but often we also want to give proper information about errors as well.
...
export async function validateInput(ctx: Context, next: () => Promise<any>) {
